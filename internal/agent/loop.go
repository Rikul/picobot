...
func NewAgentLoop(b *chat.Hub, provider providers.LLMProvider, model string, maxIterations int, workspace string, scheduler *cron.Scheduler, mcpServers map[string]config.MCPServerConfig) *AgentLoop {
	...
	return &AgentLoop{hub: b, provider: provider, tools: reg, sessions: sm, context: ctx, memory: mem, model: model, maxIterations: maxIterations, mcpClients: mcpClients, enableToolActivity: true, timeout: config.AgentTimeout(0)}
}
...
// SetAgentTimeout sets the per-message wall-clock timeout used by Run.
// Non-positive durations fall back to the default (5 minutes).
func (a *AgentLoop) SetAgentTimeout(d time.Duration) {
	if d <= 0 {
		d = config.AgentTimeout(0)
	}
	a.timeout = d
}
...
func (a *AgentLoop) Run(ctx context.Context) {
	...
	for a.running {
		...
		case msg, ok := <-a.hub.In:
			...
			msgCtx, msgCancel := context.WithTimeout(ctx, a.timeout)
			a.handleInbound(msgCtx, msg)
			msgCancel()
			continue
		default:
			// idle tick
...
// handleInbound processes a single inbound message using ctx as the deadline
// for LLM calls and tool execution.
func (a *AgentLoop) handleInbound(ctx context.Context, msg chat.Inbound) {
	...
	for iteration < a.maxIterations {
		...
		if err != nil {
			...
			if errors.Is(err, context.DeadlineExceeded) || errors.Is(ctx.Err(), context.DeadlineExceeded) {
				finalContent = "Sorry, I timed out while processing your request."
			} else {
